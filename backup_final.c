...
void inr_forward(float mm){
	...
	while(step >= stepper_inr_count){
		if(stepper_inr_count <= step/4){
	        save1 = Map(stepper_inr_count,0,step/4,0,140);
			OCR2A = 200 - save1;
		}
			...
			save1 = Map(stepper_inr_count,step*3/4,step,140,0);
			OCR2A = 200 - save1;
		}
	}
    TIMSK2 = 0;
...
void ballseach_ver2(){
	...
	else inr_back(global_ballthrough*200 + 30);
	_delay_ms(300);
	inr_rotate_left(90);
	_delay_ms(300);
	inr_back(80);
